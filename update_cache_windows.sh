...
[ $# -eq 0 ] && { echo "Usage: $0 <full path to turtlebrowser checkout>"; exit 1; }
...
echo "TurtleBrowser dir: " ${TURTLEBROWSER__DIR}
...
confirm "Update TurtleBrowser? [y/N]" && cd ${TURTLEBROWSER_DIR} && git pull

confirm "Remove old build directory? [y/N]" && cd ${TURTLEBROWSER_DIR} && rm -rf build

confirm "Create build directory? [y/N]" && cd ${TURTLEBROWSER_DIR} && mkdir build

confirm "Start build? [y/N]" && cd ${TURTLEBROWSER_DIR} && cd build && cmake -DUPDATE_CONAN=ON -DCMAKE_BUILD_TYPE=Release ..

header "Clean Conan"

confirm "Clean Conan builds? [y/N]" && conan remove -f "*" --builds

confirm "Clean Conan builds? [y/N]" && conan remove -f "*" --src

confirm "Clean Conan builds? [y/N]" && conan remove -f "*" --system-reqs
...
confirm "TEST: Remove old build directory? [y/N]" && cd ${TURTLEBROWSER_DIR} && rm -rf build

confirm "TEST: Create build directory? [y/N]" && cd ${TURTLEBROWSER_DIR} && mkdir build

confirm "TEST: Start REGULAR build? [y/N]" && cd ${TURTLEBROWSER_DIR} && cd build && cmake -DUPDATE_CONAN=OFF -DCMAKE_BUILD_TYPE=Release ..
